...
def _drive_to_done(loop, diff, test_output, *, task_type="api", variant="v1"):
    ...
    assert loop.submit(t.id, "IMPLEMENT", diff).action == LoopAction.RUN_PHASE
...
def test_bake_gate_registry_blocks_console_log(loop):
    ...
    assert loop._gate_pass[t.id] is False
...
def test_bake_scope_measurement_flags_creep(loop):
    ...
    assert loop._gate_pass[t.id] is True
    assert loop._scope_clean[t.id] is False         # measurement caught it
...
def test_bake_intent_mismatch_blocks_loop_at_implement(loop):
    ...
    try:
        ...
        assert outcome.action == LoopAction.RETRY
        assert loop._scope_clean[t.id] is False
    finally:
...
def test_bake_sha_gate_blocks_faked_output(loop):
    ...
    assert loop._test_pass[t.id] is False
...
def test_bake_sha_gate_passes_real_output(loop):
    ...
    assert loop._test_pass[t.id] is True
...
def _bake_report():
    ...
    loop.submit(t.id, "IMPLEMENT", CLEAN); loop.submit(t.id, "CONTEXT_GUARD", "ok")
